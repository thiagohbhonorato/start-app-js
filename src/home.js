(function(global, $) {
	...
	Home.prototype.initHead = function () {
		...
		this.$topBarToggle  = $("<div class='topbar-toggle' data-toggle='tooltip' data-placement='bottom' data-delay='500' title='Menu lateral'><i class='fa fa-bars'></i></div>");
		...
		if ( this.json.config.leftbarOpen || this.json.config.leftbarBeginOpen ) {
			...
			!this.json.config.topbarItemsVisible && this.$topBarTitle.addClass("invisibility");
		}
		...
		var $navTitle = $("<nav class='sidebar'></nav>").appendTo( this.$topBarTitle );
		...
		$.each(this.json.data, function(){
			if ( this.type === "list" ) {
				$("<li></li>")
					.appendTo( $ulTitle )
					.append( sprintf("<a href='%s'><span>%s</span></a>",treatLink(this.link),this.name));
			}
		});
	...
	Home.prototype.initMenu = function () {
		this.$leftBarContainer = $("<div class='leftbar-container'></div>").appendTo( this.$leftBar );
		this.$leftBarLists = $("<nav class='sidebar'></nav>")
									.appendTo( $("<div/>",{"class":"leftbar-lists"}).appendTo( this.$leftBarContainer ) )
		...
		$.when.apply($, menu ).then(function() {
			that.$leftBarSettings = $("<ul></ul>").appendTo( $("<div/>",{"class":"leftbar-settings"}).appendTo( that.$leftBarContainer ) );
			
			$('<li onclick="newElement(\'list\')"></li>').appendTo(that.$leftBarSettings).append("<i class='fa fa-plus'></i><span>Criar novo bloco</span>");
			$("<li onclick='upload()'></li>").appendTo(that.$leftBarSettings).append("<i class='fa fa-arrow-up up'></i><span>Importar</span>");
			$("<li onclick='download()'></li>").appendTo(that.$leftBarSettings).append("<i class='fa fa-arrow-down down'></i><span>Exportar</span>");
			$("<li onclick='settings()'></li>").appendTo(that.$leftBarSettings).append("<i class='fa fa-cog'></i><span>Configurações</span>");
		});
	};
	...
	Home.prototype.initBody = function () {
		...
		this.$chooserImg = $("<input type='file' id='chooserImage' onchange='changeImage(event)' accept='image/*' style='display:none;'/>").appendTo( this.$body );
		this.$openFile   = $("<input type='file' id='openFile' onchange='openFile()' accept='.home' style='display:none;'/>");
	};
	...
	Home.prototype.initEvents = function () {
		...
		this.$topBarToggle.off("click").on("click", $.proxy(function() {
			this.$body.toggleClass("open-toggle");
			this.json.config.leftbarSave && this.saveLeftbar();
			if ( !this.json.config.topbarItemsVisible ) 
				this.$body.hasClass("open-toggle") && this.$topBarTitle.addClass("invisibility") || this.$topBarTitle.removeClass("invisibility");
		}, this));
		
		// Evento para alterar o título de cada lista
		$(document).off("click", ".list-name").on("click", ".list-name", function(e) {
			var $this = $(this);
			...
			$input.on("blur", function(e){
				...
				if ( this.value !== valueOld ) {
					$this.attr("id", that.utils.removerAcentos( this.value ).toLowerCase() );
					that.saveData();
				}
        ...
        var loadBlocks = function() {
            ...
            $.when.apply($, $.map(that.json.data, function(item, i){
                if ( item.type == "list" )
                    items["sub"+i] = {
                        name: item.name, 
                        icon: "fa-check",
                        callback: function() {
                            that.move(this, $(item.link).parents(".list").find(".list-body"));
                        }
                    };
            })).then(function(){
                dfd.resolve(items);
                    ...
                    items: {
                        ...
                        "redim": {
                            ...
                            "items": {
                                ...
                                "2": {"name": "2 Coluna", "icon": "edit", "disabled": disabledResize},
                                "3": {"name": "3 Coluna", "icon": "edit", "disabled": disabledResize},
                                "4": {"name": "4 Coluna", "icon": "edit", "disabled": disabledResize},
                                "5": {"name": "5 Coluna", "icon": "edit", "disabled": disabledResize},
                                "6": {"name": "6 Coluna", "icon": "edit", "disabled": disabledResize},
                                "7": {"name": "7 Coluna", "icon": "edit", "disabled":function(){return window.innerWidth < 1600}},
                                "8": {"name": "8 Coluna", "icon": "edit", "disabled":function(){return window.innerWidth < 1600}}
                            }
                        }
                    }
                }
		...
		this.$btnPlus.off("click").on("click", function(e) {
			...
			var $item = $list.parents(".list").children(".list-body").children().last();
		...
		global.onbeforeunload = function () {
			if (!global.downloaded)
				return "As alterações não foram salvas em disco.";
		};
	...
	Home.prototype.createMenu = function ( $ul, data ) {
		...
		var map = $.map(data, function( el ) {
			...
			var $li = $("<li></li>").appendTo( $ul ),
				$link = $("<a href='"+treatLink(el.link)+"'></a>").appendTo( $li ).text( el.name );
			
			el.type == "list" 
				&& that.createMenu( $("<ul class='nav'></ul>").appendTo( $li ), el.children )
	...
	Home.prototype.generatorEl = function (item) {
		if ( item.type === "item" ) {
			...
			var $item = $("<div class='list'></div>"),
				...
				$menu = $("<div class='list-menu'><i class='fa fa-ellipsis-h'></i></div>"),
	...
	Home.prototype.saveLeftbar = function () {
		var leftbarOpen = this.json.config.leftbarSave && this.$body.hasClass("open-toggle");
	...
	Home.prototype.saveConfig = function ( settings ) {
		...
		var leftbarOpen = settings.leftbarSave && this.$body.hasClass("open-toggle");
	...
	Home.prototype.settings = function() {
		...
		}).appendTo( $form ).css(style);
		
        //-------------------------------------------------------
        
		var $opt2 = $("<div/>").inputSwitch({
			name: "leftbarSave",
